...
@interface OAuthController ()<WKNavigationDelegate,WKUIDelegate>
{
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [kNotificationCenter addObserver:self selector:@selector(keyboardFrameChange) name:UIKeyboardDidChangeFrameNotification object:nil];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if (range.length != 0) {
        ...
        NSString *haveCode = [url substringFromIndex:fromIndex];
        NSRange stateRange = [haveCode rangeOfString:@"&state"];

        
        NSString * code    =  [haveCode substringToIndex:stateRange.location];
        
        // 利用code换取一个accessToken
        
        NSLog(@"授权码%@==+++%@",code,haveCode);
...
- (void)accessTokenWithCode:(NSString*)code
{
    ...
    CZNetWorkTool * manger   = [CZNetWorkTool sharedManager];
    
    //    设置请求头
    [manger.requestSerializer setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
    NSString  * requestUrl   = [NSString stringWithFormat:@"%@%@",URL_main,URL_ACCESS_TOKEN];
    
    [manger requestWithMethod:POST WithPath:requestUrl WithParams:params WithSuccessBlock:^(NSDictionary *dic) {
        [MBProgressHUD hideHUD];
        
        NSLog(@"用户信息==%@",dic);
        
        [[UserManger sharedUserManger]saveUserMsgWith:dic];
        
        //登录成功，
        KPostNotification(KNotificationLoginStateChange, @YES)
        
    } WithFailurBlock:^(NSError *error) {
           NSLog(@"error11==%@",error.localizedDescription);
        //没有登录过，展示登录页面
